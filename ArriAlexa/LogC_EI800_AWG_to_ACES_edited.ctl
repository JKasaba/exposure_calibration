...
const float exposure_normalization = 0.3827;
...
{
    float lin_AWG[3];
    lin_AWG[0] = LogC_to_lin( rIn) * (exposure_normalization / .391);
    lin_AWG[1] = LogC_to_lin( gIn) * (exposure_normalization / .391);
    lin_AWG[2] = LogC_to_lin( bIn) * (exposure_normalization / .391);

    //lin_AWG[0] = LogC_to_lin(rIn);
    //lin_AWG[1] = LogC_to_lin(gIn);
    //lin_AWG[2] = LogC_to_lin(bIn);

    // Debugging output for linear values
    //print("Log c", LogC_to_lin());
